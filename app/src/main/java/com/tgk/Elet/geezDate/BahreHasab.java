...
import com.tgk.Elet.localDate.DaysOfWeek;
...
public class BahreHasab extends GeezYear {
    ...
    static final int TINTE_METQIE = 19, TINTE_ABEQTIE = 11, GREAT_FAST = 14, MOUNT_OF_OLIVES = 41;
    static final int PALM_SUNDAY = 62, GOOD_FRIDAY = 67, EASTER = 69, CONGRESS_OF_THE_SAGES = 93;
    static final int ASCENSION = 108, PARACLETE = 118, FAST_OF_APOSTLES = 119, FAST_OF_SALVATION = 121;
    ...
    public HolyDay[] ofYear(){
        return new HolyDay[]{getMetqie(),getNineveh(),getHoliday(GREAT_FAST,5)
                ,getHoliday(BahreHasab.MOUNT_OF_OLIVES,6),getHoliday(PALM_SUNDAY,7)
    ...
    public static HolyDay[] getAll(int year){
        BahreHasab hassab = new BahreHasab(year);
        ArrayList<HolyDay> list = new ArrayList<>();
        list.add(new HolyDay(year,1,1, String.valueOf(DaysOfWeek.values()[(hassab.getBahti()+1)%7])));
        list.addAll(Arrays.asList(hassab.ofYear()));
        return list.toArray(new HolyDay[0]);
    }
}